...
export const Repositories = ({ repos }) => {
  ...
  const selectedFirstRepo = repos.find(
    (repo) => repo.name === selectedFirstRepoName,
  );
  const selectedSecondRepo = repos.find(
    (repo) => repo.name === selectedSecondRepoName,
  );
  const otherRepos = repos.filter(
    (repo) =>
      repo.name !== selectedFirstRepoName &&
      repo.name !== selectedSecondRepoName,
  );
    ...
    <ul className="grid mid:grid-cols-2 mid:gap-8 p-0 grid-cols-1 gap-4">
      {orderedRepos.map((repo) => (
        <li
          ...
          key={repo.id}
        >
          <h3 className="m-0 text-blueElementColor text-2xl">{repo.name}</h3>
          ...
          <div className="my-4 mx-0 ">
            Demo:
            <a
              className="text-blueElementColor border-b border-blue-800 text-left my-0 mx-2"
